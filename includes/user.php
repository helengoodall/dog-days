...
class user{
	...
	public function user_data(){
			...
			if(isset($_SESSION['user_id'])){
				...
				include("connect.php");
				$get = $db->query("SELECT * FROM user WHERE user_id='".$user_id."'");
				$querydata = $get->fetch_assoc();
	...
	public function register_user($userdata){
		...
		if(isset($email)&&isset($fname)&&isset($lname)&&isset($pass)){
			...
			if($checkemail->num_rows < 1){
				...
				$regUser = $db->query("INSERT INTO user (first_name, last_name, email, password, address1, address2, city, postcode, country) 
				VALUES ('".$fname."','".$lname."','".$email."','".$pass."','".$address1."','".$address2."','".$city."','".$postcode."','".$country."')");
		...
		if($email!=""&&$pass!=""){
			...
			if($dbuser['email']==$email&&$dbuser['password']==$pass){
				...
				if(isset($_SESSION['bag'])){
					...
					$basket = $this->transfer_basket($_SESSION['bag']);
	...
	private function transfer_basket($bag){
		...
		for ($i = 0; $i < count($bag); $i++) {
			...
			$itemid = $bag[$i]["product_id"];
			$quantity = $bag[$i]["quantity"];
			$date = time();
			
			$findbasket = $db->query("SELECT quantity FROM basket WHERE user_id='".$userid."' AND product_id='".$itemid."'");
			
			if(($findbasket->num_rows) > 0){
				
				$basket = $findbasket->fetch_assoc();
				$oldquan = $basket["quantity"];
				$newquan = $oldquan + $quantity;
				
				$update = $db->query("UPDATE basket SET quantity='".$newquan."' WHERE user_id='".$userid."' AND product_id='".$itemid."'");
				
			}else{
				
				$insert = $db->query("INSERT INTO basket (product_id, user_id, quantity, date_added) VALUES ('".$itemid."', '".$userid."', '".$quantity."', '".$date."')");
